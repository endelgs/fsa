...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
		...
		<div class="alinhamento">
			<b class="alinhamento">Paciente:</b><br/>
			<?php 
		          		...
		          		'options'=>array(
		            		...
		            		'showAnim'=>'fold',
		               		...
		               		'select'=>'js:function(event, ui) {
		          				$("#AgendaTriagem_paciente_r").val(ui.item.id);//linha mto importante é o que faz funcionar o autocomplete heheheh
		          				/*
...
<h3>Triagens agendadas para hoje</h3>
<table>
	<tr>
		<th>Paciente</th>
...
<?php $aTriagensHoje=$model->findAll('data=:data', array(':data'=>date("Y-m-d")));
foreach($aTriagensHoje as $triagem){
	echo '<tr class="itemAgenda">
		<td>'.CHtml::encode($triagem->pacienteR->nome).'</td>
		<td>'.CHtml::encode($triagem->data).'</td>
		<td>'.CHtml::encode($triagem->horario).'</td>
...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<h3>Triagens agendadas entre:</h3>
	<?php $this->widget('zii.widgets.jui.CJuiDatePicker', array(
	        'name'=>'data_ini',
	        'language'=>'pt-BR',
	       	'options'=>array(
	...
	?>
	<?php echo CHtml::ajaxSubmitButton('Pesquisar',array('agendaTriagem/getTriagens'),array('update'=>'#boxTriagens',));?>
	<div id="boxTriagens"></div>
<?php $this->endWidget(); ?>
</div><!-- search-form -->
...
<script>
	$("#agenda_hoje").click(function(){
		...
		if($(this).attr('checked')=="checked")$("#AgendaTriagem_data").val(hoje);
		else $("#AgendaTriagem_data").val("");
	});
	$("#AgendaTriagem_data").change(function(){
		var data = new Date();
		var mes=(data.getMonth()+1)<=9?"0"+(data.getMonth()+1):(data.getMonth()+1);
		var hoje=dia = data.getDate()+"/"+mes+"/"+data.getFullYear();
