...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
		<div class="alinhamento">
			<b>Agendar Diagnósticos para:</b><br/>
		          		...
		          		'options'=>array(
		            		...
		            		'showAnim'=>'fold',
		               		...
		               		'select'=>'js:function(event, ui) {
		          				$("#AgendaDiagnostico_paciente_r").val(ui.item.id);//linha mto importante é o que faz funcionar o autocomplete heheheh
		          				/*
...
<h3>Diagnósticos agendados para hoje</h3>
<table>
	<tr>
		<th>Paciente</th>
...
<?php $aDiagnosticosHoje=$model->findAll('data=:data', array(':data'=>date("Y-m-d")));
foreach($aDiagnosticosHoje as $diagnostico){
	echo '<tr class="itemAgenda">
		<td>'.CHtml::encode($diagnostico->pacienteR->nome).'</td>
		<td>'.CHtml::encode($diagnostico->data).'</td>
		<td>'.CHtml::encode($diagnostico->horario).'</td>
...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<h3>Diagnósticos agendados entre:</h3>
	<?php $this->widget('zii.widgets.jui.CJuiDatePicker', array(
	        'name'=>'data_ini',
	        'language'=>'pt-BR',
	       	'options'=>array(
	...
	?>
	<?php echo CHtml::ajaxSubmitButton('Pesquisar',array('agendaDiagnostico/getDiagnosticos'),array('update'=>'#boxDiagnosticos',));?>
	<div id="boxDiagnosticos"></div>
<?php $this->endWidget(); ?>
</div><!-- search-form -->
...
<script>
	$("#agenda_hoje").click(function(){
		...
		if($(this).attr('checked')=="checked")$("#AgendaDiagnostico_data").val(hoje);
		else $("#AgendaDiagnostico_data").val("");
	});
	$("#AgendaDiagnostico_data").change(function(){
		var data = new Date();
		var mes=(data.getMonth()+1)<=9?"0"+(data.getMonth()+1):(data.getMonth()+1);
		var hoje=dia = data.getDate()+"/"+mes+"/"+data.getFullYear();
